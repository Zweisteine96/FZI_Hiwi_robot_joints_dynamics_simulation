...
class TargetPublisher {
 ...
 public:
  ...
  void getPartInfo(const box_info_msgs::BoxInformationStamped::ConstPtr& part_msg)
  {
    ...
    if( (robot_target_.pose.position.x < x_min_) || (robot_target_.pose.position.x > x_max_) || (robot_target_.pose.position.y < y_min_) || (robot_target_.pose.position.y > y_max_) )
    {
      return;
    }
